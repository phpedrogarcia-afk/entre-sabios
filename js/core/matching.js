...
function getSelectionSignature(state, genderPreference) {
  return [state.primaryFeeling, ...(state.secondaryFeelings || []).slice().sort(), state.intensity, genderPreference].join('|');
}
...
function pickRuntimeContent() {
  ...
  const signature = getSelectionSignature(state, currentGenderPreference);
  ...
  if (!result) {
    recordEditorialSignal('contentGap', state, { genderPreference: currentGenderPreference });
    return null;
  }

// Dados extraídos de script.js durante a Fase 2 da refatoração segura.
// Não alterar conteúdo editorial nesta fase.
(function initEntreSabiosData(root) {
  const data = root.EntreSabiosData = root.EntreSabiosData || {};

data.philosophicalTales = [
  {
    id: 'flecha-envenenada',
    titulo: 'A Flecha Envenenada',
    origem: 'Adaptação editorial do Cūḷamāluṅkya Sutta (MN 63)',
    tradicao: 'Budismo antigo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['ansiedade', 'medo', 'confusão'],
    temas: ['presente', 'controle', 'urgência', 'sofrimento', 'clareza'],
    palavrasChave: ['ansiedade', 'futuro', 'pergunta', 'dor', 'atenção'],
    texto: [
      'A flecha entrou abaixo do ombro e derrubou o homem. Quando seus parentes chegaram, encontraram-no com uma das mãos apertada contra a haste. A ponta estava coberta de veneno. Chamaram um cirurgião, que se ajoelhou ao lado dele e abriu o estojo de instrumentos.',
      '— Não toque — disse o ferido. — Primeiro quero saber quem atirou.',
      'O cirurgião pediu que ele respirasse devagar. O homem insistiu. Queria o nome e o clã do arqueiro, sua altura, a cor de sua pele, a aldeia de onde viera. Queria saber de que madeira era o arco, de que fibra a corda fora trançada e que ave cedera as penas presas à haste.',
      'Um parente correu até a estrada. Outro interrogou quem trabalhava nos campos. Voltaram com versões, não com respostas. Enquanto isso, o braço do homem começou a perder a força.',
      'O cirurgião tornou a estender a mão. Ainda havia tempo para retirar a flecha; talvez houvesse tempo para procurar o arqueiro depois. O ferido, porém, pediu mais uma certeza: queria saber a composição exata do veneno antes que qualquer lâmina tocasse sua pele.',
      'Ninguém naquela casa conhecia todas as respostas. O estojo permaneceu aberto no chão. Entre os instrumentos imóveis, a sombra da flecha avançava sobre o corpo do homem.'
    ],
    umModoDeOlhar: [
      'As perguntas do ferido não são inúteis; apenas não têm a mesma urgência da flecha. O conto põe lado a lado duas necessidades legítimas — compreender o ataque e preservar a vida — e mostra o perigo de tratá-las como se devessem ser satisfeitas na ordem errada.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Há momentos em que procurar uma explicação completa oferece a sensação de recuperar controle. Ainda assim, o que é desconhecido pode continuar desconhecido enquanto algo imediato pede atenção. A inteligência também se perde quando só aceita agir depois de eliminar toda incerteza.'
    ],
    perguntaParaLevar: 'O que, em uma situação ainda aberta, precisa ser atendido antes de ser plenamente explicado?',
  },
  {
    id: 'mito-da-caverna',
    titulo: 'O Mito da Caverna',
    origem: 'Adaptação editorial de Platão, A República, livro VII',
    tradicao: 'Filosofia grega',
    tempoLeitura: 3,
    sentimentosRelacionados: ['autoconhecimento', 'confusão', 'medo', 'insegurança'],
    temas: ['verdade', 'aparência', 'consciência', 'liberdade', 'autoengano'],
    palavrasChave: ['autoconhecimento', 'ilusão', 'clareza', 'sombra', 'verdade'],
    imagem: {
      src: 'assets/contos/alegoria-da-caverna-piloto.webp',
      alt: 'Silhuetas contemplam sombras no interior de uma caverna enquanto um caminho conduz à luz exterior.',
      width: 1536,
      height: 864,
    },
    texto: [
      'Platão pede que imaginemos homens vivendo desde o nascimento no fundo de uma caverna. Eles não caminham livremente. Estão presos de tal modo que só conseguem olhar para a parede diante de si. Atrás deles há uma fogueira, e entre a fogueira e os prisioneiros passam pessoas carregando objetos. Tudo o que os prisioneiros veem são sombras projetadas na pedra.',
      'Como nunca conheceram outra coisa, chamam aquelas sombras de realidade. Observam suas formas, disputam seus nomes, tentam prever seus movimentos. Talvez alguns sejam admirados por reconhecer uma sombra antes dos outros. Dentro da caverna, essa habilidade parece sabedoria. Ninguém ali percebe que está discutindo apenas o contorno escuro de algo que não vê diretamente.',
      'Um dia, um dos prisioneiros é libertado. A primeira experiência não é agradável. Seu corpo dói, seus olhos recusam a claridade, e tudo aquilo que antes parecia certo começa a se desfazer. Ao virar-se para a fogueira, percebe que as sombras tinham uma origem. Ao subir para fora da caverna, a luz do sol o fere ainda mais. No começo, ele distingue apenas reflexos na água; depois, aos poucos, enxerga objetos, seres vivos, o céu e finalmente a própria fonte da luz.',
      'A educação, nessa imagem, não parece um simples acúmulo de informações. Parece uma conversão do olhar. O prisioneiro não recebe apenas novas respostas; ele descobre que havia aprendido a chamar de realidade uma versão empobrecida do mundo. Cada passo para fora exige abandonar uma certeza que antes lhe dava lugar entre os outros.',
      'Quando compreende o que viu, não consegue guardar aquilo apenas para si. Retorna à caverna para contar aos outros que o mundo é maior do que a parede. Mas seus olhos, acostumados à claridade, demoram a se adaptar novamente à escuridão. Para os prisioneiros, ele parece desorientado. O relato da luz soa absurdo para quem passou a vida inteira chamando sombras de mundo.',
      'A volta é talvez a parte mais difícil da história. Ver mais não garante ser compreendido. Quem se acostumou ao escuro pode defender a própria prisão como se ela fosse segurança. A libertação, em Platão, não é apenas sair da caverna; é suportar a dor de enxergar e ainda assim tentar viver de acordo com o que foi visto.'
    ],
    umModoDeOlhar: [
      'A alegoria não opõe simplesmente ignorância e conhecimento. O prisioneiro libertado precisa reaprender a ver, e seu retorno mostra que uma descoberta também pode romper a linguagem partilhada com quem ficou. A luz traz responsabilidade, não apenas vantagem.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Algumas certezas parecem naturais porque nunca foram observadas de outro lugar. Quando uma delas se desfaz, o desconforto não prova que a nova visão seja falsa; pode ser apenas o intervalo em que os olhos ainda procuram medida.'
    ],
    perguntaParaLevar: 'Que certeza mudaria de forma se pudesse ser observada de outro ponto?',
  },
  {
    id: 'mito-de-narciso',
    titulo: 'O Mito de Narciso',
    origem: 'Adaptação editorial de Ovídio, Metamorfoses, livro III',
    tradicao: 'Tradição clássica',
    tempoLeitura: 2,
    sentimentosRelacionados: ['amor', 'solidão', 'raiva', 'autoconhecimento', 'insegurança'],
    temas: ['vaidade', 'orgulho', 'imagem', 'aprovação', 'vazio'],
    palavrasChave: ['narciso', 'orgulho', 'vaidade', 'aprovação', 'espelho'],
    texto: [
      'Eco o viu entre as árvores quando ele separava os cães depois da caça. Quis chamá-lo, mas uma antiga condenação lhe permitia apenas devolver as últimas palavras que ouvia. Esperou que Narciso falasse primeiro.',
      '— Há alguém aqui? — perguntou ele.',
      '— Aqui — respondeu ela, saindo do bosque com os braços abertos. Narciso recuou. Preferia, disse, morrer a se entregar a alguém. Eco repetiu o fim da frase, mas ele já se afastava. Restou-lhe a voz, cada vez mais fraca, presa às palavras dos outros.',
      'Não foi a única rejeição. Rapazes e moças procuraram Narciso e encontraram o mesmo desprezo. Um deles pediu aos deuses que o jovem conhecesse um desejo impossível de alcançar. Nêmesis ouviu.',
      'Num meio-dia de caça, cansado e com sede, Narciso encontrou uma fonte que nenhum animal havia turvado. Ajoelhou-se. Sob a água clara apareceu um rosto que não desviava os olhos. Ele sorriu; o rosto sorriu. Aproximou os lábios; a superfície se rompeu.',
      'Narciso esperou a água aquietar. Tentou tocar os cabelos, a boca, o pescoço. Cada gesto desfazia o que desejava, e cada espera o devolvia mais perfeito. Chamou pela figura. A mata respondeu com a voz de Eco, mas do fundo da fonte não veio palavra alguma.',
      'Só depois reconheceu os próprios olhos. A descoberta não o libertou: tornou exata a impossibilidade. Não podia abandonar o que amava sem abandonar a própria imagem, nem podia abraçá-la sem destruí-la na água. Permaneceu ali enquanto a força deixava seu corpo.',
      'Quando as ninfas vieram buscá-lo, não encontraram o jovem. À margem da fonte havia apenas uma flor de centro dourado, inclinada sobre o mesmo espelho.'
    ],
    umModoDeOlhar: [
      'O reflexo oferece a Narciso uma presença sem alteridade: acompanha cada gesto, nunca discorda e nunca chega de fato. Eco, ao contrário, deseja encontrá-lo, mas sua voz depende das palavras dele. Entre os dois, o mito aproxima formas diferentes de uma relação que não consegue acontecer.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Ser admirado não é o mesmo que ser encontrado. A imagem pode proteger da surpresa, do limite e da resposta imprevisível de outra pessoa; por isso também pode fechar o desejo num circuito onde nada novo entra.'
    ],
    perguntaParaLevar: 'Que diferença existe, hoje, entre o olhar que você procura e o encontro que poderia surpreendê-lo?',
  },
  {
    id: 'agricultor-e-o-cavalo',
    titulo: 'O Agricultor e o Cavalo',
    origem: 'Adaptação editorial do Huainanzi, capítulo 18 (塞翁失馬)',
    tradicao: 'Tradição chinesa',
    tempoLeitura: 3,
    sentimentosRelacionados: ['medo', 'ansiedade', 'esperança', 'confusão'],
    temas: ['incerteza', 'julgamento', 'tempo', 'mudança', 'aceitação'],
    palavrasChave: ['medo', 'sorte', 'azar', 'cavalo', 'paciência'],
    texto: [
      'Na borda norte da aldeia, onde os campos terminavam junto às terras de fronteira, um agricultor abriu o curral antes do amanhecer e encontrou a tranca no chão. O único cavalo da família havia desaparecido. Restavam marcas de casco seguindo para além das colinas.',
      'Os vizinhos vieram ajudá-lo a procurar. Ao anoitecer, regressaram sem o animal e se sentaram diante da casa. Disseram que perder um cavalo às vésperas do plantio era uma desgraça. O agricultor passou a mão pela madeira quebrada da tranca. — Como podemos saber? — respondeu. Depois consertou o curral vazio.',
      'Alguns dias mais tarde, um tropel levantou poeira na estrada. O cavalo voltava acompanhado de vários animais selvagens. A aldeia cercou o curral, contando quantos poderiam ser vendidos ou postos no arado. Agora chamavam o agricultor de afortunado. Ele observou o filho, que já escolhia com os olhos uma égua ainda arisca. — Como podemos saber? — repetiu.',
      'O rapaz começou a domar os animais. Numa manhã, a égua se ergueu sobre as patas traseiras e o lançou contra uma pedra. O estalo foi ouvido do outro lado da cerca. O pai correu, cortou a bota inchada e imobilizou a perna com duas ripas enquanto o filho mordia um pedaço de couro.',
      'Os vizinhos trouxeram caldo e lamentaram a nova infelicidade. Sem o rapaz, o plantio seria lento; talvez ele nunca voltasse a andar direito. O agricultor agradeceu a comida, trocou a faixa ensanguentada e disse apenas que ainda não podiam ver onde aquele acontecimento terminava.',
      'Antes que o osso se firmasse, soldados chegaram com uma lista. A guerra avançava na fronteira, e cada família deveria entregar um jovem capaz de marchar. Portas se abriram, nomes foram chamados, sacos foram postos às costas. Quando o oficial viu a perna presa às ripas, riscou o nome do filho e seguiu para a casa seguinte.',
      'Naquela noite, ninguém veio anunciar sorte ou desgraça. O agricultor levou uma tigela ao filho, conferiu a tranca do curral e ficou escutando os cavalos se moverem no escuro. A estrada por onde os outros rapazes haviam partido já não levantava poeira.'
    ],
    umModoDeOlhar: [
      'A resposta do agricultor não transforma dor em bênção nem perigo em promessa. Ela apenas recusa um veredito antes que as consequências tenham terminado de se formar. Enquanto os vizinhos nomeiam cada mudança, ele atende ao que está diante dele: a cerca, o ferimento, a comida, a noite.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Incerteza e passividade não são a mesma coisa. É possível cuidar do que aconteceu sem decidir depressa o que aquilo significará para sempre. Alguns fatos pedem ação imediata; o nome definitivo pode esperar.'
    ],
    perguntaParaLevar: 'Que acontecimento ainda pode mudar de sentido com o tempo?',
  },
  {
    id: 'dois-monges-e-a-mulher',
    titulo: 'Os Dois Monges e a Mulher',
    origem: 'Adaptação editorial de “Muddy Road”, história associada a Tanzan e Ekido',
    tradicao: 'Zen japonês',
    tempoLeitura: 2,
    sentimentosRelacionados: ['raiva', 'culpa', 'amor', 'saudade'],
    temas: ['apego', 'ressentimento', 'desapego', 'julgamento', 'leveza'],
    palavrasChave: ['apego', 'culpa', 'raiva', 'soltar', 'julgamento'],
    texto: [
      'A chuva ainda caía quando Tanzan e Ekido chegaram a uma curva alagada. A lama cobria a passagem de um muro ao outro. Junto à beirada, uma mulher de quimono claro media o chão com os olhos; qualquer passo a faria afundar até os tornozelos.',
      'Tanzan parou diante dela.',
      '— Venha — disse. Antes que Ekido perguntasse o que pretendia, tomou a mulher nos braços, atravessou a lama e a deixou no trecho seco. Ela agradeceu. Tanzan inclinou a cabeça e retomou o caminho.',
      'Ekido veio atrás. Nos primeiros passos, esperou uma explicação. Depois, esperou um pedido de desculpas. Os dois atravessaram campos, passaram por uma aldeia e dividiram a água de uma cabaça. Tanzan falava apenas do abrigo onde poderiam passar a noite.',
      'Quando avistaram o portão do templo, a chuva havia parado. Ekido já não conseguiu conter a pergunta.',
      '— Fomos ensinados a não tocar em mulheres, sobretudo numa jovem como aquela. Como pôde carregá-la?',
      'Tanzan pousou a cabaça no degrau e olhou para o companheiro.',
      '— Eu a deixei naquela curva — disse. — Você ainda está com ela?'
    ],
    umModoDeOlhar: [
      'Tanzan responde à dificuldade que encontra; Ekido permanece fiel à regra, mas transforma a espera por uma explicação em acusação. O conto não resolve de forma abstrata a tensão entre disciplina e compaixão. Deixa que os dois modos de agir caminhem lado a lado até o portão.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Um acontecimento pode terminar no mundo e continuar ocupando espaço por dentro. Às vezes, o peso não está apenas no que ocorreu, mas na conversa adiada, na sentença repetida e na necessidade de manter o outro preso àquela cena.'
    ],
    perguntaParaLevar: 'O que mantém uma cena antiga presente, mesmo quando o caminho já seguiu adiante?',
  },
  {
    id: 'caixa-de-pandora',
    titulo: 'A Caixa de Pandora',
    origem: 'Adaptação editorial de Hesíodo, Os Trabalhos e os Dias, vv. 90–105',
    tradicao: 'Tradição clássica',
    tempoLeitura: 2,
    sentimentosRelacionados: ['esperança', 'tristeza', 'luto', 'medo'],
    temas: ['esperança', 'sofrimento', 'perda', 'permanência', 'fragilidade'],
    palavrasChave: ['pandora', 'esperança', 'dor', 'mundo', 'caixa'],
    texto: [
      'No relato de Hesíodo, Pandora levanta a grande tampa de um jarro. A imagem que se tornou famosa como “caixa” começa, na verdade, com um recipiente de boca larga e uma casa recém-habitada por humanos.',
      'Assim que a tampa se abre, males até então contidos ganham o ar: doença, fadiga, velhice, inquietação. Não saem um a um, como visitantes que pudessem ser chamados de volta. Espalham-se depressa e passam a circular sem voz entre os mortais.',
      'Pandora baixa a tampa antes que uma última presença atravesse a borda. Elpis — palavra muitas vezes traduzida por esperança — fica no fundo do jarro, sob a tampa. O poema não a entrega ao mundo nem explica por inteiro o que sua permanência significa.',
      'Do lado de fora, a vida humana já não é a mesma. Há trabalho, doença e perda. Do lado de dentro, algo continua guardado. O jarro fechado sustenta as duas imagens ao mesmo tempo, sem escolher por nós se aquilo que restou é proteção, espera ou uma promessa ainda suspensa.'
    ],
    umModoDeOlhar: [
      'A força do mito está na ambiguidade do que permanece. O sofrimento não é desfeito, e a esperança tampouco é apresentada como solução pronta. Entre o que escapou e o que ficou, o jarro recusa uma leitura simples da condição humana.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Esperar pode ser muito diferente de negar a dor. Há esperanças que não prometem retorno ao que era intacto; apenas ajudam a não transformar o sofrimento na única linguagem disponível.'
    ],
    perguntaParaLevar: 'O que a esperança guarda quando não se confunde com promessa?',
  },
  {
    id: 'sonho-da-borboleta',
    titulo: 'O Sonho da Borboleta',
    origem: 'Adaptação editorial de Zhuangzi, capítulo 2 (齐物论)',
    tradicao: 'Taoismo',
    tempoLeitura: 1,
    tempoLeituraTexto: '1 minuto',
    sentimentosRelacionados: ['confusão', 'autoconhecimento', 'ansiedade'],
    temas: ['identidade', 'percepção', 'mistério', 'leveza', 'realidade'],
    palavrasChave: ['borboleta', 'sonho', 'confusão', 'identidade', 'realidade'],
    texto: [
      'Zhuang Zhou sonhou que era uma borboleta. Voava contente, seguindo o próprio movimento, sem saber que era Zhou.',
      'No sonho, não havia filósofo, biografia ou pergunta. Havia apenas a borboleta, inteira em seu voo.',
      'Quando acordou, Zhou estava ali outra vez. Mas a certeza de ter voltado a si já não era tão simples. Teria Zhou sonhado que era borboleta? Ou uma borboleta sonhava agora que era Zhou?',
      'O relato termina nessa passagem entre uma forma e outra. Em vez de fixar qual delas é a verdadeira, chama esse deslocamento de transformação das coisas.'
    ],
    umModoDeOlhar: [
      'A brevidade do sonho é parte de sua força. Ele não prova que a identidade seja ilusória; interrompe a pressa de tratá-la como uma coisa imóvel e inteiramente dominada pelo pensamento.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Há perguntas que perdem algo quando recebem resposta cedo demais. A borboleta não oferece uma técnica para acabar com a confusão; deixa espaço para perceber que mudança e estranhamento também fazem parte de estar vivo.'
    ],
    perguntaParaLevar: 'O que muda quando a pergunta sobre quem somos não precisa ser resolvida depressa?',
  },
  {
    id: 'nachiketa-e-yama',
    titulo: 'Nachiketa e Yama',
    origem: 'Adaptação editorial da Kaṭha Upaniṣad, 1.1–1.2',
    tradicao: 'Sabedoria indiana',
    tempoLeitura: 3,
    sentimentosRelacionados: ['falta_de_proposito', 'coragem', 'medo', 'autoconhecimento'],
    temas: ['propósito', 'morte', 'escolha', 'disciplina', 'sentido'],
    palavrasChave: ['propósito', 'morte', 'yama', 'coragem', 'sentido'],
    texto: [
      'Nachiketa aparece na Katha Upanishad como um jovem levado a fazer uma pergunta que muitos adultos evitam. Depois de um conflito com o pai, é enviado simbolicamente à morada de Yama, o senhor da morte. Ao chegar, não encontra o anfitrião. Permanece ali por três noites, esperando sem ser recebido, diante da casa daquele que governa o limite final de todos os seres.',
      'Quando Yama retorna, reconhece que o jovem hóspede foi deixado sem acolhimento. Para reparar a falha, oferece três dádivas. Nachiketa não pede qualquer coisa de imediato. Primeiro deseja a reconciliação com o pai, para que o retorno à casa não seja marcado por rancor. Depois pede conhecimento sobre o fogo sagrado, sinal de disciplina e caminho espiritual.',
      'A terceira dádiva é a mais difícil. Nachiketa pergunta o que acontece depois da morte. Quer saber se há algo que permanece quando o corpo termina, algo que não seja destruído pelo tempo. Yama tenta desviá-lo. Oferece riquezas, vida longa, prazeres, reinos, música, poder e tudo aquilo que costuma seduzir o desejo humano. O jovem escuta, mas percebe que essas ofertas pertencem ao mesmo mundo que passa.',
      'Nachiketa não rejeita a vida por desprezo. Rejeita apenas a troca da verdade por distrações brilhantes. Sabe que prazeres envelhecem, posses mudam de mãos e uma vida longa continua terminando diante da morte. O que ele procura não é mais acúmulo, mas direção.',
      'Nachiketa não luta contra Yama com força, nem tenta vencer a morte como inimigo. Permanece diante dela sem se vender ao medo nem ao encanto das recompensas.',
      'Nachiketa não quer escapar da morte por negociação; quer compreender a vida de tal modo que a morte não a transforme em puro desperdício.',
      'Diante de sua firmeza, Yama começa a ensinar a diferença entre o agradável e o bom. O agradável seduz rápido e pode ocupar uma vida inteira sem orientá-la; o bom exige discernimento e oferece eixo. Nachiketa se torna, assim, a imagem daquele que encara a morte não por morbidez, mas para descobrir o que merece conduzir a vida.'
    ],
    umModoDeOlhar: [
      'Yama oferece a Nachiketa tudo o que poderia fazer a terceira pergunta desaparecer sem respondê-la. A escolha do jovem não opõe prazer e virtude de maneira simples; distingue o que passa depressa daquilo que pode orientar uma vida submetida ao tempo.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'A lembrança da finitude pode reorganizar prioridades sem tornar a vida menor. Algumas distrações perdem força quando examinadas diante do limite; outras alegrias continuam valiosas justamente porque não fingem ser permanentes.'
    ],
    perguntaParaLevar: 'O que conserva importância quando você o observa à luz de sua duração?',
  },
  {
    id: 'patinho-feio',
    titulo: 'O Patinho Feio',
    origem: 'Adaptação editorial de Hans Christian Andersen, O Patinho Feio (1843)',
    tradicao: 'Conto literário europeu',
    tempoLeitura: 3,
    sentimentosRelacionados: ['solidão', 'tristeza', 'autoconhecimento', 'esperança'],
    temas: ['pertencimento', 'identidade', 'tempo', 'diferença', 'dignidade'],
    palavrasChave: ['solidão', 'patinho', 'pertencimento', 'diferença', 'cisne'],
    texto: [
      'No ninho de uma pata, um dos filhotes nasceu diferente. Era maior, estranho ao olhar dos outros, menos gracioso dentro daquele pequeno mundo que esperava semelhança. Desde cedo, sua presença parecia causar incômodo. Os outros patinhos o rejeitavam, os animais do quintal o ridicularizavam, e até os espaços que deveriam ser abrigo pareciam apertados demais para sua forma.',
      'O filhote tentou pertencer. Caminhou como podia, suportou comparações, ouviu risos e empurrões. Aos poucos, começou a acreditar que havia algo errado em existir daquele modo. A diferença, repetida pelos outros como defeito, entrou nele como vergonha. Não era apenas excluído; começava a olhar para si com os olhos de quem o feria.',
      'Sem encontrar lugar, afastou-se. Atravessou campos, frio, medo e solidão. Em alguns momentos encontrou abrigo passageiro; em outros, apenas a necessidade de continuar. O inverno tornou tudo mais duro. A vida parecia uma longa espera por uma explicação que não chegava. Ele não sabia ainda que crescer também pode doer quando ninguém reconhece aquilo que está amadurecendo.',
      'Com o tempo, aproximou-se de um lago e viu aves grandes e belas deslizando sobre a água. Eram cisnes. Diante deles, sentiu novamente o velho temor de ser rejeitado. Baixou a cabeça, esperando desprezo. Mas, ao olhar para o reflexo na água, viu algo que não esperava: sua forma havia mudado. Aquilo que antes parecia desajeitado revelava agora outra natureza.',
      'O reconhecimento não veio como aplauso barulhento. Veio primeiro como visão. A água, que em tantas histórias devolve ilusão, aqui devolve verdade. O filhote viu no próprio reflexo uma continuidade que os outros não tinham percebido: ele não havia se tornado digno apenas naquele instante; havia crescido todo o tempo em direção a uma forma que o quintal não sabia nomear.',
      'A dignidade não chegou como prêmio concedido pelos que o haviam humilhado. Diante dos cisnes, ele esperou o golpe de sempre; em vez disso, viu as aves se aproximarem sem hostilidade.',
      'Crianças vieram ao jardim e disseram que havia um cisne novo. Ele escondeu a cabeça sob a asa, confuso com a alegria. O inverno, o quintal e as fugas não desapareceram. Mas já não eram a única história que seu reflexo podia contar.'
    ],
    umModoDeOlhar: [
      'O desfecho não torna necessária a crueldade sofrida, nem prova que toda exclusão esconda uma identidade excepcional. Ele altera o peso do julgamento inicial: o quintal chamou de defeito aquilo que só sabia medir por semelhança.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Ser recebido por outro ambiente pode revelar que certos nomes impostos cedo demais não descrevem uma vida inteira. Ainda assim, reconhecer-se não apaga o frio atravessado nem exige gratidão pela rejeição.'
    ],
    perguntaParaLevar: 'Que nome recebido no passado já não alcança tudo o que você se tornou?',
  },
  {
    id: 'davi-e-golias',
    titulo: 'Davi e Golias',
    origem: 'Adaptação editorial de 1 Samuel 17',
    tradicao: 'Narrativa bíblica',
    tempoLeitura: 2,
    sentimentosRelacionados: ['coragem', 'medo', 'falta_de_proposito', 'raiva'],
    temas: ['coragem', 'proporção', 'ação', 'confiança', 'limites'],
    palavrasChave: ['coragem', 'medo', 'gigante', 'ação', 'davi'],
    texto: [
      'Por quarenta dias, Golias saiu do acampamento filisteu pela manhã e ao entardecer. Chamava os israelitas para que escolhessem um homem e decidissem a guerra num único combate. Ninguém atravessava o vale.',
      'Davi chegou ao acampamento para levar alimento aos irmãos. Ouviu o desafio e perguntou por que todos deixavam que aquela voz ocupasse o campo. Quando se ofereceu, o rei Saul tentou vesti-lo com a própria armadura. Davi caminhou alguns passos e parou: não sabia mover-se com ela.',
      'Devolveu o bronze, tomou o cajado, escolheu cinco pedras lisas no ribeiro e levou a funda que já usava como pastor. Golias viu o rapaz se aproximar e respondeu com desprezo. Entre os dois, o vale continuava aberto.',
      'Davi correu na direção do adversário. Pôs uma pedra na funda, girou-a e a lançou. Golias caiu de rosto no chão. Por um instante, os dois exércitos ficaram olhando para o mesmo ponto do vale.',
      'Então os filisteus recuaram e os israelitas avançaram. A armadura de Saul permaneceu no acampamento, sem uso. As pedras que ficaram na margem do ribeiro continuaram onde estavam.'
    ],
    umModoDeOlhar: [
      'A cena desloca a medida do confronto. Davi não vence por se tornar parecido com Golias; entra no vale com os instrumentos que conhece e recusa uma proteção que o impede de se mover.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Há situações em que a comparação faz parecer que só existe uma forma legítima de agir. O conto não transforma toda dificuldade em batalha vencível; mostra, porém, que o tamanho aparente de um obstáculo pode impor uma medida que não é a única possível.'
    ],
    perguntaParaLevar: 'Que medida de ação aparece quando a comparação deixa de comandar a cena?',
  },
  {
    id: 'anel-de-giges',
    titulo: 'O Anel de Giges',
    origem: 'Adaptação editorial de Platão, A República, livro II, 359b–360b',
    tradicao: 'Filosofia grega',
    tempoLeitura: 3,
    sentimentosRelacionados: ['raiva', 'autoconhecimento', 'confusão'],
    temas: ['moral', 'poder', 'corrupção', 'justiça', 'caráter'],
    palavrasChave: ['poder', 'invisibilidade', 'justiça', 'corrupção', 'escolha'],
    texto: [
      'Na história narrada por Glauco em A República, Giges era um pastor a serviço do rei da Lídia. Depois de uma tempestade e de um terremoto, a terra se abriu diante dele, revelando uma fenda estranha. Ao descer, encontrou coisas maravilhosas e, entre elas, um cavalo de bronze oco. Dentro havia um corpo maior que o comum, usando um anel de ouro.',
      'Giges retirou o anel e voltou à superfície. Mais tarde, sentado entre outros pastores, girou sem querer a parte do anel voltada para dentro da mão. Nesse instante, percebeu que os demais falavam dele como se não estivesse ali. Ao girá-lo de volta, tornou-se visível outra vez. Testou o objeto algumas vezes e compreendeu o poder que carregava: podia desaparecer diante dos olhos dos outros.',
      'A descoberta mudou tudo. Giges, que antes era apenas um homem comum dentro de uma ordem social maior que ele, passou a ter acesso ao tipo de liberdade que não presta contas. Usou a invisibilidade para aproximar-se da corte, seduzir a rainha, conspirar contra o rei e tomar o poder. O anel não criou nele um desejo inteiramente novo; apenas retirou os freios que o impediam de agir.',
      'Giges não se torna outro ser por magia; torna-se alguém que já não precisa temer o olhar alheio. Aquilo que antes seria impedido pela vergonha, pela lei ou pelo risco passa a parecer disponível.',
      'Platão não conta essa história para celebrar esperteza. O anel coloca uma pergunta difícil no centro da vida moral: alguém continuaria justo se pudesse cometer injustiça sem ser visto, sem punição e sem perder reputação? A invisibilidade torna inútil a máscara social. Sobra apenas aquilo que a pessoa realmente escolhe quando ninguém pode julgá-la.',
      'A prova do caráter não está nos discursos públicos, nem na imagem de honra construída diante dos outros. Está no instante secreto em que uma pessoa poderia agir sem consequência aparente e, ainda assim, precisa decidir que tipo de alma quer alimentar.'
    ],
    umModoDeOlhar: [
      'Glauco apresenta o anel como um teste contra a ideia de que a justiça seja desejada por si mesma. A invisibilidade não prova automaticamente que todo ser humano agiria como Giges; ela retira reputação e punição da cena para tornar visível o que sustentava cada escolha.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Há decisões cujo limite parece vir apenas do risco de exposição. Observar esse limite não exige presumir uma natureza secreta e corrupta; permite perguntar que vínculos, valores ou consequências continuam presentes quando o olhar público sai de cena.'
    ],
    perguntaParaLevar: 'O que sustentaria uma escolha sua se ela permanecesse inteiramente sem testemunhas?',
  },
  {
    id: 'mito-de-er',
    titulo: 'O Mito de Er',
    origem: 'Adaptação editorial de Platão, A República, livro X, 614b–621d',
    tradicao: 'Filosofia grega',
    tempoLeitura: 2,
    sentimentosRelacionados: ['falta_de_proposito', 'medo', 'esperança', 'autoconhecimento'],
    temas: ['vida', 'morte', 'escolhas', 'destino', 'responsabilidade'],
    palavrasChave: ['morte', 'destino', 'escolha', 'alma', 'vida'],
    texto: [
      'No fim de A República, Platão apresenta a história de Er, um guerreiro que caiu em batalha. Seu corpo foi recolhido entre os mortos, mas, dias depois, quando estava prestes a ser cremado, ele voltou à vida. Ao despertar, contou o que havia visto enquanto estivera fora do mundo comum.',
      'Er descreveu uma região onde as almas eram julgadas e seguiam caminhos diferentes conforme a vida que haviam levado. Algumas subiam, outras desciam, carregando consequências de suas ações. Depois de um longo percurso, retornavam a um lugar onde deveriam escolher uma nova existência. Diante delas apareciam muitos destinos possíveis: vidas de poder, beleza, riqueza, simplicidade, sofrimento, grandeza ou obscuridade.',
      'A cena decisiva não era imposta por uma força cega. As almas escolhiam. Algumas, ainda seduzidas por aparências, tomavam rapidamente uma vida brilhante sem perceber os males escondidos nela. Outras escolhiam com mais cuidado, lembrando dores anteriores e examinando melhor aquilo que parecia desejável.',
      'Ulisses recebeu o último lote. Depois de procurar longamente, encontrou a vida discreta de um homem particular, deixada de lado pelos demais. Disse que a teria escolhido mesmo se seu lote fosse o primeiro.',
      'As almas atravessaram a planície do Esquecimento e beberam da água do rio Ameles. Quem não guardou medida esqueceu tudo. Er não bebeu. No meio da noite, um trovão lançou cada alma para um novo nascimento.',
      'Ele não soube dizer por qual caminho voltou ao corpo. Abriu os olhos ao amanhecer, deitado sobre a pira funerária, com a lembrança que deveria contar aos vivos.'
    ],
    umModoDeOlhar: [
      'O sorteio define a ordem, não a vida escolhida. Platão combina acaso, responsabilidade e formação: a liberdade existe, mas pode ser mal usada por uma alma que não aprendeu a examinar o brilho de cada destino.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Escolher envolve mais que desejar uma aparência de vida. Experiências anteriores, hábitos e atenção alteram o que cada pessoa consegue reconhecer nas opções diante de si — inclusive os custos que ficam escondidos.'
    ],
    perguntaParaLevar: 'Que aspecto de uma escolha costuma ficar invisível quando a possibilidade parece brilhante?',
  },
  {
    id: 'sisifo',
    titulo: 'Sísifo',
    origem: 'Adaptação editorial do mito grego de Sísifo, com referência a Albert Camus, O Mito de Sísifo',
    tradicao: 'Filosofia do absurdo',
    tempoLeitura: 3,
    sentimentosRelacionados: ['falta_de_proposito', 'tristeza', 'coragem', 'confusão'],
    temas: ['sentido da vida', 'persistência', 'absurdo', 'rotina', 'revolta'],
    palavrasChave: ['sísifo', 'absurdo', 'pedra', 'persistência', 'sentido'],
    texto: [
      'Sísifo, na mitologia grega, era conhecido por sua astúcia. Enganou deuses, desafiou limites e tentou escapar da ordem imposta até mesmo pela morte. Sua inteligência, que tantas vezes lhe dera vantagem, tornou-se também a razão de sua punição. No mundo dos mortos, recebeu uma tarefa sem descanso: empurrar uma enorme pedra montanha acima.',
      'O esforço era real. A pedra tinha peso, o corpo se inclinava, o caminho subia. Sísifo precisava avançar passo a passo até quase alcançar o cume. Mas, no instante em que a conclusão parecia próxima, a pedra escapava e rolava de volta ao vale. Tudo recomeçava. Não havia progresso acumulado, chegada definitiva ou testemunha capaz de transformar aquilo em glória.',
      'À primeira vista, é uma imagem de castigo absoluto. O trabalho existe, mas não produz conclusão. A força é exigida, mas não recebe repouso. O tempo passa sem oferecer a recompensa que normalmente usamos para suportar o esforço. O absurdo nasce justamente dessa distância entre a necessidade humana de sentido e a repetição indiferente da tarefa.',
      'Qualquer pessoa que já se sentiu presa a uma rotina sem fim reconhece algo dessa montanha. A mesma dificuldade retorna, o mesmo peso pede as mesmas forças, e a promessa de conclusão parece se afastar no momento exato em que parecia próxima. O mito transforma essa experiência em imagem pura: uma pedra, uma subida, uma queda, um retorno.',
      'Albert Camus, ao reler esse mito, desloca o olhar para um momento específico: a descida. Depois que a pedra rola, Sísifo caminha novamente até o vale. Nesse intervalo, ele sabe tudo. Sabe que a pedra voltará a cair, sabe que a tarefa não terminará, sabe que nenhuma ilusão o espera no alto. E ainda assim retorna.',
      'A grandeza da imagem não está em fingir que a pedra é leve. Está em encarar seu peso sem entregar a consciência. Sísifo não vence a montanha no sentido comum; sua revolta está em continuar lúcido. A pedra permanece absurda, mas ele deixa de ser apenas objeto do castigo quando percebe sua condição e caminha de volta sem se ajoelhar interiormente.'
    ],
    umModoDeOlhar: [
      'O mito oferece a punição; Camus concentra sua leitura na descida, quando Sísifo conhece por inteiro a repetição que o espera. A interpretação não torna o castigo bom nem a pedra desejável. Pergunta o que a consciência modifica quando não consegue modificar a tarefa.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Repetição e cansaço não precisam ser convertidos em prova de grandeza. Nomear o peso pode impedir tanto a negação quanto a romantização, deixando aberta a questão sobre onde ainda existe margem de resposta.'
    ],
    perguntaParaLevar: 'Em que momento de uma repetição você percebe com mais clareza o que ela faz com você?',
  },
  {
    id: 'icaro',
    titulo: 'Ícaro',
    origem: 'Adaptação editorial de Ovídio, Metamorfoses, livro VIII',
    tradicao: 'Tradição clássica',
    tempoLeitura: 3,
    sentimentosRelacionados: ['coragem', 'ansiedade', 'falta_de_proposito', 'confusão'],
    temas: ['ambição', 'prudência', 'limites', 'desejo', 'queda'],
    palavrasChave: ['ícaro', 'ambição', 'limite', 'sol', 'prudência'],
    texto: [
      'Dédalo era um artesão engenhoso, preso em Creta com seu filho Ícaro. A saída por terra e por mar estava bloqueada. Restava imaginar um caminho onde ninguém esperava fuga: o ar. Com penas, fios e cera, Dédalo construiu dois pares de asas. Não eram asas divinas; eram uma obra frágil, nascida da inteligência humana diante do confinamento.',
      'Antes de partir, advertiu o filho. Se voasse baixo demais, a umidade do mar pesaria as penas. Se subisse alto demais, o calor do sol derreteria a cera. A liberdade dependia de uma medida: nem a proximidade perigosa das águas, nem a vertigem de tocar o céu. O caminho seguro era o meio, sustentado por atenção.',
      'No começo, o voo era travessia. Pai e filho deixavam para trás o labirinto, a prisão, o impossível. Mas Ícaro sentiu a embriaguez da altura. O ar abriu nele uma alegria nova, e a advertência começou a parecer pequena diante da sensação de subir. Quanto mais alto voava, mais o limite parecia uma voz distante.',
      'É importante notar que Ícaro não cai por desejar liberdade. Ele cai quando confunde liberdade com esquecimento da própria condição. As asas eram reais, mas frágeis. Funcionavam porque uniam invenção e cuidado. Quando o encanto da altura apagou a memória da cera, o voo deixou de ser caminho e virou excesso.',
      'A cera então amoleceu. As penas se soltaram uma a uma. O corpo que experimentava a leveza encontrou de repente a queda. Ícaro caiu no mar que depois levaria seu nome. Dédalo continuou, marcado pela perda e pela lembrança amarga de que a inteligência que abre caminhos não elimina a necessidade de prudência.',
      'A cena final guarda uma dor dupla: a queda do filho e a permanência do pai. Dédalo sobrevive ao voo que inventou, mas sua invenção fica para sempre ligada ao limite que não foi respeitado. O mito conserva essa tensão entre criatividade e medida, desejo e consequência.',
      'A história de Ícaro não condena o desejo de voar. Sem esse desejo, não haveria fuga do labirinto. O que ela questiona é o instante em que liberdade se transforma em fascínio por ultrapassar qualquer medida. Há alturas que libertam; há alturas que apenas provam que esquecemos a fragilidade das asas.'
    ],
    umModoDeOlhar: [
      'As asas são ao mesmo tempo solução e fragilidade. Dédalo não aconselha imobilidade: pede uma rota entre o mar e o sol. A tragédia nasce dentro de uma fuga bem-sucedida, quando a experiência do voo deixa de conservar memória dos materiais que a tornam possível.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Uma conquista pode alterar a percepção dos limites que ainda a sustentam. Reconhecer cera e penas não diminui a invenção; mantém visível a diferença entre ampliar uma possibilidade e agir como se ela não tivesse condições.'
    ],
    perguntaParaLevar: 'Que condição do seu voo fica mais fácil esquecer quando a altura traz entusiasmo?',
  },
  {
    id: 'prometeu',
    titulo: 'Prometeu',
    origem: 'Adaptação editorial de Hesíodo, Teogonia e Os Trabalhos e os Dias',
    tradicao: 'Tradição clássica',
    tempoLeitura: 3,
    sentimentosRelacionados: ['coragem', 'raiva', 'falta_de_proposito', 'esperança'],
    temas: ['conhecimento', 'sacrifício', 'liberdade', 'rebeldia', 'humanidade'],
    palavrasChave: ['prometeu', 'fogo', 'liberdade', 'sacrifício', 'conhecimento'],
    texto: [
      'Prometeu é lembrado na mitologia grega como aquele que favoreceu os humanos diante dos deuses. Em um mundo onde a distância entre mortais e divindades era enorme, o fogo pertencia ao alto, ao domínio sagrado, ao poder que não deveria ser simplesmente entregue. Sem ele, os homens permaneciam mais frágeis diante da noite, do frio e da natureza.',
      'Prometeu roubou o fogo e o levou aos humanos. Esse gesto não significava apenas entregar uma chama física. O fogo abria possibilidades: cozinhar, aquecer, fabricar, iluminar, transformar matéria, reunir pessoas ao redor de uma claridade comum. Com ele, a vida humana ganhava técnica, criação e autonomia.',
      'Mas aquilo que liberta também altera a ordem do poder. O gesto de Prometeu foi visto como transgressão. Por desafiar os deuses e favorecer os mortais, recebeu punição severa. Foi acorrentado, exposto, ferido repetidamente. Em versões conhecidas do mito, uma ave devora seu fígado, que volta a crescer para que o sofrimento recomece.',
      'A punição repetida torna visível o tamanho do conflito. Prometeu não sofre por um erro pequeno, mas por ter deslocado uma fronteira. Ao entregar o fogo, aproximou os humanos de uma potência antes reservada aos deuses. O castigo tenta lembrar que toda ampliação de liberdade ameaça alguma ordem estabelecida.',
      'A imagem é dura porque une dádiva e custo. Prometeu não entrega aos humanos uma facilidade sem consequência. Entrega uma força capaz de iluminar e queimar. O mesmo fogo que aquece pode destruir; a mesma técnica que emancipa pode dominar; o mesmo conhecimento que amplia a vida exige responsabilidade.',
      'Os humanos, por sua vez, deixam de ser apenas criaturas expostas. Ao redor do fogo, podem transformar a noite, preparar alimento, criar instrumentos e imaginar futuros. A chama muda a relação deles com o mundo. Ela não elimina a vulnerabilidade humana, mas inaugura uma nova capacidade de responder a ela.',
      'Por isso Prometeu permanece como figura ambígua e poderosa. Não é apenas rebelde, nem apenas mártir. É aquele que aceita o preço de tornar possível algo maior para os humanos. Seu mito lembra que toda conquista de liberdade carrega uma pergunta: o que faremos com a chama depois de recebê-la?'
    ],
    umModoDeOlhar: [
      'As fontes antigas não apresentam apenas uma celebração do conhecimento. O fogo roubado pertence a uma disputa entre Prometeu e Zeus e vem acompanhado por punições dirigidas ao titã e aos humanos. Dádiva, transgressão e poder permanecem entrelaçados.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Uma técnica pode ampliar a capacidade humana sem determinar o uso que receberá. A chama aquece, transforma e destrói; reconhecer essa ambivalência evita tanto tratá-la como salvação quanto reduzi-la à culpa de ter sido roubada.'
    ],
    perguntaParaLevar: 'Que responsabilidades surgem junto com uma capacidade nova, além do benefício que ela promete?',
  },
  {
    id: 'navio-de-teseu',
    titulo: 'O Navio de Teseu',
    origem: 'Adaptação editorial de Plutarco, Vida de Teseu, 23, com a variante de Thomas Hobbes, De Corpore, 11.7',
    tradicao: 'Filosofia da identidade',
    tempoLeitura: 3,
    sentimentosRelacionados: ['autoconhecimento', 'confusão', 'saudade', 'luto', 'insegurança'],
    temas: ['identidade', 'mudança', 'tempo', 'memória', 'continuidade'],
    palavrasChave: ['identidade', 'mudança', 'teseu', 'tempo', 'memória'],
    texto: [
      'Durante gerações, os atenienses conservaram no porto a embarcação em que, segundo a tradição, Teseu regressara de Creta. Nos dias de cerimônia, a cidade ainda a enfeitava. Crianças tocavam o casco escuro; homens idosos apontavam os remos e contavam uma viagem que nenhum deles havia visto.',
      'O sal, porém, não respeitava lembranças. Quando um carpinteiro encontrou podridão sob a linha d’água, retirou uma prancha e colocou madeira nova no lugar. O navio continuou recebendo visitantes. No inverno seguinte, trocaram duas peças da armação. Depois vieram um mastro, parte do convés, os remos gastos. Cada reparo era pequeno o bastante para que a embarcação saísse do estaleiro com o mesmo nome.',
      'Os anos transformaram o trabalho em ofício hereditário. Um filho assumiu as ferramentas do pai; mais tarde, outro filho recebeu as mesmas medidas riscadas numa tábua. Peça após peça, a madeira que tocara o mar de Teseu foi retirada. Ainda assim, quando alguém perguntava onde estava o navio, todos apontavam para o mesmo ancoradouro.',
      'Os filósofos começaram a discutir diante do casco. Alguns diziam que a continuidade dos reparos preservava a embarcação: ela nunca deixara de ser cuidada, nomeada e reconhecida pela cidade. Outros apanhavam um fragmento descartado e perguntavam quanto poderia ser substituído antes que a homenagem passasse a pertencer a outra coisa.',
      'Séculos depois, uma nova hipótese tornou a disputa mais difícil. Imaginou-se que alguém houvesse guardado cada prancha removida. Se as peças antigas fossem reunidas na mesma ordem e voltassem a formar um casco, haveria então duas embarcações diante da cidade: uma com a história contínua dos reparos; outra com a matéria que atravessara o primeiro mar.',
      'Na manhã imaginada desse encontro, as duas proas receberiam o mesmo nome. Uma teria marcas recentes de ferramentas e velas prontas para uso. A outra traria cicatrizes de sal nas tábuas antigas. Quem procurasse apenas a madeira caminharia para um lado. Quem seguisse a continuidade do cuidado caminharia para o outro.',
      'O porto não ofereceria uma terceira placa capaz de encerrar a disputa. A maré subiria sob os dois cascos, movendo-os por igual, enquanto cada observador decidia o que havia reconhecido.'
    ],
    umModoDeOlhar: [
      'As duas embarcações concentram critérios diferentes de identidade. Uma conserva a continuidade do nome, do lugar e do cuidado; a outra reúne a matéria antiga. O paradoxo permanece porque cada resposta protege algo que parece essencial e abandona outra parte da história.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Mudanças profundas podem tornar insuficiente a pergunta “ainda é o mesmo?”. Às vezes é preciso observar quais continuidades importam, quais peças não podem voltar e quem participa do reconhecimento de uma vida através do tempo.'
    ],
    perguntaParaLevar: 'Em que você reconhece continuidade quando quase tudo parece ter mudado?',
  },
  {
    id: 'xicara-de-cha',
    titulo: 'A Xícara de Chá',
    origem: 'Adaptação editorial de história zen de circulação moderna, associada a Nan-in',
    tradicao: 'Zen-budismo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['confusão', 'autoconhecimento', 'ansiedade'],
    temas: ['humildade', 'aprendizado', 'vazio', 'escuta', 'mente'],
    palavrasChave: ['xícara', 'chá', 'humildade', 'aprender', 'mente cheia'],
    texto: [
      'Um estudioso procurou um mestre zen para receber ensinamentos. Chegou trazendo perguntas, mas também muitas respostas prontas. Enquanto o mestre o recebia, o visitante falava sobre o que já havia lido, as ideias que dominava, os caminhos espirituais que conhecia e as conclusões a que tinha chegado. Cada silêncio parecia para ele uma oportunidade de acrescentar mais alguma certeza.',
      'O mestre ouviu sem pressa. Em vez de discutir, preparou chá. Colocou a xícara diante do visitante e começou a servir. O líquido subiu até a borda, mas o mestre continuou despejando. O chá transbordou, escorreu pelo lado da xícara e começou a se espalhar pela mesa.',
      'O visitante, surpreso, interrompeu. Observou que a xícara já estava cheia, que não cabia mais nada ali. O gesto parecia absurdo: qualquer pessoa podia ver que continuar servindo apenas desperdiçaria o chá.',
      'O mestre pousou o bule. Disse que o visitante, como a xícara, estava cheio das próprias opiniões. Antes que algo novo pudesse entrar, seria preciso haver espaço.',
      'O estudioso olhou para o chá espalhado. Chegara preparado para anotar uma resposta; diante dele havia apenas a mesa molhada e a xícara que ele mesmo declarara impossível de encher.'
    ],
    umModoDeOlhar: [
      'A demonstração não condena conhecimento. Ela expõe uma forma de escuta na qual cada palavra nova é imediatamente usada para confirmar o que já estava decidido. A xícara torna concreto o limite de uma conversa sem espaço para surpresa.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Estar disponível não significa apagar o que se sabe. Pode significar apenas suspender por um momento a necessidade de encaixar tudo em conclusões anteriores e notar o que não cabe nelas.'
    ],
    perguntaParaLevar: 'Em que conversa suas respostas costumam chegar antes do que está sendo dito?',
  },
  {
    id: 'barco-vazio',
    titulo: 'O Barco Vazio',
    origem: 'Adaptação editorial de Zhuangzi, capítulo 20, “A árvore na montanha”',
    tradicao: 'Taoismo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['raiva', 'confusão', 'ansiedade'],
    temas: ['raiva', 'ego', 'julgamento', 'projeção', 'desapego'],
    palavrasChave: ['barco', 'raiva', 'ego', 'vazio', 'choque'],
    texto: [
      'Zhuangzi propõe uma cena: um homem atravessa um rio quando outro barco deriva em sua direção. Ele tenta desviar, mas os cascos se chocam.',
      'Se o barco que o atingiu está vazio, mesmo um homem irritadiço não encontra contra quem gritar. O impacto aconteceu; a corrente apenas trouxe madeira contra madeira.',
      'Se há alguém no outro barco, porém, ele chama uma vez para que mude o rumo. Chama de novo. Na terceira vez, chegam insultos. O dano pode ser o mesmo, mas agora existe um rosto capaz de receber a raiva.',
      'A passagem não pergunta quem venceu a discussão no rio. Volta-se para quem consegue atravessar o mundo como um barco vazio: sem fazer da própria importância uma carga que colide com tudo.',
      'A corrente segue. Um barco ocupado e outro vazio podem deixar marcas semelhantes no casco; não deixam necessariamente a mesma história na mente de quem foi atingido.'
    ],
    umModoDeOlhar: [
      'A imagem desloca a atenção do culpado para a presença do eu que se sente atingido. O vazio não apaga o choque nem oferece regra para toda injustiça; mostra como a mesma colisão adquire outra intensidade quando se torna afronta pessoal.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Entre um acontecimento e a reação a ele, a intenção atribuída ao outro pode ocupar quase toda a cena. Distinguir dano, descuido e afronta não elimina a necessidade de resposta, mas muda aquilo a que se responde.'
    ],
    perguntaParaLevar: 'O que muda em um conflito quando você separa o impacto da intenção que atribuiu a ele?',
  },
  {
    id: 'kisa-gotami',
    titulo: 'A Mostarda de Kisa Gotami',
    origem: 'Adaptação editorial da tradição comentarial theravāda sobre Kisā Gotamī',
    tradicao: 'Budismo antigo',
    tempoLeitura: 3,
    sentimentosRelacionados: ['luto', 'tristeza', 'saudade', 'amor'],
    temas: ['luto', 'aceitação', 'perda', 'humanidade', 'impermanência'],
    palavrasChave: ['luto', 'mostarda', 'perda', 'aceitação', 'morte'],
    texto: [
      'Kisa Gotami perdeu seu filho pequeno. A dor foi tão grande que sua mente se recusou a aceitar o acontecimento. Tomada pelo desespero, caminhou com a criança nos braços procurando alguém que pudesse devolver-lhe a vida. Pedia remédio, ajuda, qualquer coisa que desfizesse aquilo que parecia impossível de suportar.',
      'As pessoas viam sua dor e não sabiam como alcançá-la. Alguns percebiam que a criança havia morrido, mas Kisa ainda buscava cura como se houvesse uma porta secreta por onde a perda pudesse ser desfeita. Por fim, chegou até o Buda. Em vez de confrontá-la diretamente com uma explicação dura, ele lhe deu uma tarefa.',
      'Kisa deveria conseguir um grão de mostarda em uma casa onde ninguém tivesse morrido. A mostarda era comum; encontrá-la não seria difícil. O impossível estava na segunda condição. Mesmo assim, ela saiu. Bateu em portas, entrou em casas, contou sua necessidade. Em muitos lugares havia mostarda. Mas em cada casa havia também uma história de morte: um pai, uma mãe, um filho, um irmão, alguém amado que já havia partido.',
      'Ao longo da busca, a dor de Kisa mudou de lugar. Não desapareceu. Mas deixou de parecer uma condenação isolada lançada apenas sobre ela. Em cada casa, encontrou uma ferida diferente e reconheceu que a perda atravessa todos os vivos. Sua tragédia era única em amor, mas não era única na condição humana.',
      'Essa descoberta não veio como ideia abstrata. Veio pelo contato com rostos, histórias e silêncios. Cada porta mostrava que a morte não era uma exceção monstruosa reservada apenas à sua casa. Era uma verdade que atravessava todas as casas, mesmo aquelas onde a vida continuava com tarefas comuns, comida no fogo e vozes ao redor.',
      'A mostarda comum tornava a lição ainda mais forte: aquilo que todos tinham era fácil de encontrar; uma casa sem perda, não.',
      'Quando a noite chegou, Kisa já não procurava o mesmo remédio. Compreendeu que não havia casa sem morte, nem vida humana sem impermanência. O caminho de porta em porta fez aquilo que uma explicação direta talvez não conseguisse fazer: abriu espaço para a aceitação sem negar a profundidade do amor.'
    ],
    umModoDeOlhar: [
      'A condição ligada à mostarda transforma uma busca por remédio em encontro com outras casas. A compreensão não chega como argumento capaz de resolver o luto, mas como conhecimento adquirido de porta em porta: nenhuma família está fora da perda.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Uma perda permanece singular sem precisar ser solitária. Perceber que outras casas também conhecem a morte não reduz o vínculo interrompido; apenas modifica o isolamento em torno dele.'
    ],
    perguntaParaLevar: 'O que se torna diferente quando uma dor encontra lugar entre as histórias de outras pessoas?',
  },
  {
    id: 'arvore-inutil',
    titulo: 'A Árvore Inútil',
    origem: 'Adaptação editorial de motivos da “árvore inútil” em Zhuangzi, capítulos 1 e 4',
    tradicao: 'Taoismo',
    tempoLeitura: 3,
    sentimentosRelacionados: ['falta_de_proposito', 'autoconhecimento', 'solidão', 'confusão'],
    temas: ['valor', 'utilidade', 'descanso', 'existência', 'liberdade'],
    palavrasChave: ['árvore', 'inútil', 'valor', 'utilidade', 'descanso'],
    texto: [
      'Nas histórias associadas a Zhuangzi, aparece uma árvore enorme, antiga e irregular. Seus galhos cresciam de modo estranho, seu tronco não seguia a linha desejada pelos carpinteiros, e sua madeira parecia inadequada para tábuas, vigas ou móveis. Quem passava avaliando utilidade logo a desprezava. Para os olhos práticos, era uma árvore sem valor.',
      'Carpinteiros podiam olhar para ela e concluir que não servia. Não valia o esforço de cortá-la, porque sua madeira torta não obedeceria aos projetos humanos. Ela não se encaixava em medidas, ferramentas e expectativas. Parecia grande demais para ser ignorada e inútil demais para ser aproveitada.',
      'Justamente por isso, permaneceu viva. Árvores retas e úteis eram cortadas, transformadas, consumidas por finalidades alheias. A árvore considerada inútil continuava crescendo. Seus galhos ofereciam sombra, seu corpo abrigava descanso, sua presença criava um espaço onde viajantes podiam parar. Aquilo que não servia ao mercado da madeira servia à vida de outro modo.',
      'A história não elogia incapacidade nem romantiza abandono. Ela questiona um critério estreito de valor. Se algo não serve a uma função imediata, isso não significa que não tenha sentido. Pode apenas escapar ao olhar de quem só reconhece valor quando consegue usar, vender, cortar ou controlar.',
      'A árvore não precisa convencer os carpinteiros. Ela simplesmente permanece. Sua existência larga demais para as ferramentas humanas torna-se abrigo justamente porque não foi reduzida a material. Aquilo que parecia defeito — ser torta, irregular, difícil de aproveitar — tornou-se condição de sobrevivência.',
      'Há uma ironia calma nessa imagem: o que o mundo chama de inútil talvez esteja apenas protegido de certas formas de exploração. Nem toda vida precisa ser convertida em produto para justificar sua presença.',
      'Quem descansa sob sua sombra talvez receba algo que nenhuma tábua poderia oferecer.',
      'Zhuangzi transforma a árvore em uma defesa da existência que não precisa justificar tudo em termos de produtividade. Às vezes, sobreviver é possível justamente porque não fomos transformados em ferramenta. A sombra que oferecemos talvez não apareça nas métricas de utilidade, mas ainda assim pode ser real.'
    ],
    umModoDeOlhar: [
      'Esta adaptação reúne imagens próximas de Zhuangzi: a árvore que o carpinteiro descarta e a madeira tortuosa que não se presta aos usos comuns. A inutilidade não vira uma essência superior; é uma relação entre a forma da árvore e os projetos de quem pretende cortá-la.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Um critério de utilidade pode revelar o que procura e permanecer cego ao que não sabe aproveitar. Escapar desse critério também tem ambiguidade: protege a árvore do machado, mas não promete reconhecimento.'
    ],
    perguntaParaLevar: 'Que possibilidade aparece quando algo deixa de ser avaliado apenas pelo uso esperado dele?',
  },
  {
    id: 'carruagem-da-mente',
    titulo: 'A Carruagem da Mente',
    origem: 'Adaptação editorial da Kaṭha Upaniṣad, 1.3.3–9',
    tradicao: 'Hinduísmo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['ansiedade', 'raiva', 'medo', 'confusão'],
    temas: ['controle da mente', 'emoções', 'disciplina', 'direção', 'autodomínio'],
    palavrasChave: ['carruagem', 'mente', 'emoções', 'controle', 'direção'],
    texto: [
      'Na Kaṭha Upaniṣad, Yama oferece a Nachiketa uma imagem para compreender o ser humano: o corpo é uma carruagem e o eu que a ocupa é o passageiro.',
      'O intelecto ocupa o lugar do condutor. A mente são as rédeas em suas mãos. Os sentidos são os cavalos, e aquilo que eles percebem forma as estradas por onde a carruagem avança.',
      'Quando o condutor não discerne e a mente não oferece direção, os sentidos se tornam difíceis de governar, como cavalos sem treino. A carruagem é levada pelos caminhos que se apresentam.',
      'Quando há discernimento e as rédeas permanecem reunidas, os cavalos respondem ao percurso. A imagem não exige destruir sentidos ou abandonar a estrada; depende da relação entre todas as partes.',
      'Nenhum elemento viaja sozinho. Passageiro, veículo, condutor, rédeas, cavalos e caminhos compõem um movimento cujo destino não pode ser atribuído a uma única força.'
    ],
    umModoDeOlhar: [
      'A imagem distribui a direção entre funções diferentes. A mente não é o condutor nem os cavalos são emoções em geral: no texto, ela liga o discernimento aos sentidos. Confundir as partes simplifica uma metáfora construída justamente sobre coordenação.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Alguns movimentos parecem vir de uma força única, mas dependem da relação entre atenção, discernimento, sentidos, corpo e ambiente. Observar essa relação pode ser mais preciso do que procurar um culpado dentro da carruagem.'
    ],
    perguntaParaLevar: 'Qual relação entre as partes da carruagem parece mais decisiva no percurso que você observa agora?',
  },
  {
    id: 'filho-prodigo',
    titulo: 'O Filho Pródigo',
    origem: 'Adaptação editorial do Evangelho de Lucas 15:11–32',
    tradicao: 'Cristianismo',
    tempoLeitura: 3,
    sentimentosRelacionados: ['culpa', 'tristeza', 'esperança', 'amor'],
    temas: ['perdão', 'recomeço', 'retorno', 'dignidade', 'acolhimento'],
    palavrasChave: ['perdão', 'culpa', 'recomeço', 'filho', 'retorno'],
    texto: [
      'Na parábola do Evangelho de Lucas, um filho mais novo pede ao pai a parte da herança que lhe caberia. O pedido já carrega ruptura: ele deseja receber como se a relação pudesse ser antecipada, repartida e levada embora. Depois de obter o que queria, parte para longe. Em uma terra distante, gasta tudo. O que antes parecia liberdade se transforma em abandono, fome e vergonha.',
      'Quando a necessidade chega ao ponto mais baixo, ele começa a lembrar da casa do pai. Não imagina voltar exigindo o lugar antigo. Prepara dentro de si um discurso humilde, disposto a pedir apenas uma posição de servo. A culpa já não aparece como palavra bonita; aparece como consciência da distância que ele mesmo ajudou a criar.',
      'Mas a volta não acontece como ele esperava. Antes que consiga completar sua explicação, o pai o vê de longe e vai ao seu encontro. A narrativa não apresenta o pai como alguém indiferente ao erro, mas como alguém que se recusa a deixar o erro ser a identidade final do filho. O retorno é acolhido antes de ser negociado em detalhes. O gesto do pai interrompe a fantasia de que toda queda precisa terminar em exílio permanente.',
      'Há festa, roupa nova, sinal de dignidade restaurada. Mas a história não termina apenas com reconciliação. O filho mais velho, que permaneceu em casa, sente-se ferido. Para ele, a alegria pelo retorno do irmão parece injustiça. Sua obediência escondia uma expectativa de recompensa e talvez uma solidão que nunca tinha sido dita.',
      'O pai também sai para encontrar o filho mais velho. Lembra que tudo o que possui está com ele, mas diz que era preciso alegrar-se porque o irmão estava perdido e foi encontrado.',
      'A parábola termina antes de dizer se o filho mais velho entra na festa. De um lado chegam a música e a dança; do outro, permanece a resposta que ele ainda precisa dar.'
    ],
    umModoDeOlhar: [
      'A história contém dois movimentos do pai para fora de casa: ele corre ao encontro de quem retorna e vai procurar quem se recusa a entrar. O acolhimento do filho mais novo não resolve a comparação do mais velho; abre um segundo conflito que o texto deixa sem resposta.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Voltar não restaura uma casa intacta. O retorno encontra alegria, memória da ruptura e ressentimento convivendo no mesmo espaço. Ser acolhido e reparar o que aconteceu são movimentos relacionados, mas não idênticos.'
    ],
    perguntaParaLevar: 'Em qual limiar desta história — partir, voltar ou entrar na festa — você encontra a tensão mais viva?',
  },
  {
    id: 'bom-samaritano',
    titulo: 'O Bom Samaritano',
    origem: 'Adaptação editorial do Evangelho de Lucas 10:25–37',
    tradicao: 'Cristianismo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['amor', 'raiva', 'solidão', 'tristeza'],
    temas: ['empatia', 'cuidado', 'próximo', 'ação', 'compaixão'],
    palavrasChave: ['empatia', 'cuidado', 'samaritano', 'compaixão', 'ferida'],
    texto: [
      'Na parábola do Bom Samaritano, um homem desce de Jerusalém para Jericó e cai nas mãos de assaltantes. É agredido, roubado e deixado à beira do caminho, ferido demais para seguir sozinho. A estrada continua existindo ao redor dele: pessoas passam, passos se aproximam e se afastam, a vida parece seguir apesar do corpo caído.',
      'Primeiro passa alguém ligado ao serviço religioso. Vê o homem, mas segue adiante. Depois passa outro, também associado a uma posição respeitada, e também continua o caminho. A narrativa não se demora explicando seus motivos. Talvez houvesse medo, pressa, regras de pureza, conveniência ou simples distância interior. O fato permanece: eles viram e passaram.',
      'Então aparece um samaritano, figura que, para os ouvintes originais da história, não seria o herói esperado. Ele se aproxima do ferido, trata suas feridas, coloca-o sobre seu animal, leva-o a uma hospedaria e providencia cuidado. A compaixão, aqui, não é uma emoção abstrata. Ela toma tempo, usa recursos, interrompe a própria rota. O cuidado começa quando a agenda de alguém aceita ser atravessada pela vulnerabilidade de outro.',
      'Na hospedaria, o samaritano continuou cuidando dele. No dia seguinte, entregou duas moedas ao hospedeiro e pediu que mantivesse o cuidado; se gastasse mais, pagaria quando voltasse.',
      'Ao terminar, Jesus não perguntou qual personagem pertencia à categoria correta. Perguntou qual dos três havia se tornado próximo do homem ferido. O interlocutor respondeu: aquele que usou de misericórdia.'
    ],
    umModoDeOlhar: [
      'A pergunta inicial procura delimitar quem conta como próximo. A parábola responde mudando o verbo: o samaritano não é identificado previamente como membro do grupo certo; torna-se próximo por uma sequência concreta de aproximação, cuidado e compromisso.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Ver a ferida e aproximar-se são acontecimentos diferentes. A narrativa não explica por que os dois primeiros passaram ao largo, nem transforma ajuda em impulso sem custo: há interrupção, risco, dinheiro e continuidade.'
    ],
    perguntaParaLevar: 'Onde, nesta história, termina a identificação de um próximo e começa o ato de tornar-se próximo?',
  },
  {
    id: 'os-talentos',
    titulo: 'Os Talentos',
    origem: 'Adaptação editorial do Evangelho de Mateus 25:14–30',
    tradicao: 'Cristianismo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['falta_de_proposito', 'medo', 'coragem', 'culpa'],
    temas: ['potencial', 'responsabilidade', 'medo', 'ação', 'crescimento'],
    palavrasChave: ['talentos', 'potencial', 'responsabilidade', 'medo', 'ação'],
    texto: [
      'Na parábola dos Talentos, um senhor parte em viagem e confia seus bens a servos diferentes. A cada um entrega uma quantidade conforme sua capacidade. Não se trata apenas de dinheiro na história; trata-se de confiança, responsabilidade e possibilidade de ação durante a ausência do senhor.',
      'Alguns servos recebem o que lhes foi entregue e fazem aquilo crescer. Trabalham, arriscam, movimentam os recursos. Quando o senhor retorna, apresentam não apenas conservação, mas desenvolvimento. Aquilo que receberam não ficou congelado no medo de perda.',
      'Outro servo, porém, age de modo diferente. Recebe um talento e o enterra. Seu raciocínio parece prudente: ao esconder, não perde; ao não tocar, não erra; ao não arriscar, evita culpa. Mas sua segurança é estéril. O talento permanece intacto e inútil, protegido de tal maneira que nunca encontrou vida. Ele não destruiu o que recebeu; ainda assim, deixou que aquilo atravessasse o tempo sem produzir presença no mundo.',
      'Quando o senhor retorna, o problema não é apenas a quantidade final. O problema é a relação do servo com aquilo que recebeu. Ele viveu como se responsabilidade fosse ameaça pura. Em vez de responder à confiança com movimento, respondeu com imobilidade. O medo de falhar tornou-se uma falha mais profunda.',
      'O servo devolve a moeda e explica que teve medo: conhecia o senhor como homem severo, que colhia onde não havia semeado. Por isso escondera no chão o que recebera.',
      'O senhor usa as próprias palavras do servo contra ele. Diz que, se o julgava tão exigente, poderia ao menos ter confiado o dinheiro aos banqueiros. Manda retirar dele o talento e entregá-lo ao servo que tem dez.',
      'A história termina com uma sentença dura: o servo é lançado para fora. O dinheiro enterrado volta à circulação; o medo que o preservou não o protege do acerto de contas.'
    ],
    umModoDeOlhar: [
      'No texto, talento é uma grande soma de dinheiro, não uma habilidade pessoal. A leitura moderna sobre capacidades é uma interpretação posterior. No centro da parábola estão confiança, risco, medo e prestação de contas sob a autoridade de um senhor descrito como severo.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'O medo pode conservar intacto aquilo que foi confiado e, ainda assim, impedir qualquer relação viva com essa confiança. A dureza do desfecho também impede transformar a história em incentivo leve à produtividade.'
    ],
    perguntaParaLevar: 'Que diferença existe, para você, entre preservar algo confiado e mantê-lo enterrado?',
  },
  {
    id: 'elefante-no-escuro',
    titulo: 'O Elefante no Escuro',
    origem: 'Adaptação editorial de Rumi, Masnavi, livro III, vv. 1259–1268',
    tradicao: 'Sufismo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['confusão', 'raiva', 'autoconhecimento', 'insegurança'],
    temas: ['verdade', 'perspectiva', 'limite', 'humildade', 'diálogo'],
    palavrasChave: ['elefante', 'escuro', 'perspectiva', 'verdade', 'parte'],
    texto: [
      'Na história conhecida pelo sufismo e presente em Rumi, um elefante é colocado em um espaço escuro. As pessoas entram para conhecê-lo, mas não conseguem vê-lo por inteiro. Sem luz, cada uma toca apenas uma parte do animal e tenta compreender o todo a partir desse contato limitado.',
      'Quem toca a tromba acredita ter encontrado algo parecido com uma serpente. Quem toca a perna imagina uma coluna. Quem alcança a orelha pensa em um leque. Quem encosta no dorso percebe uma superfície ampla. Cada pessoa sai com convicção, pois sua experiência foi real. O problema é que foi incompleta.',
      'A discussão começa quando cada parte se apresenta como totalidade. Ninguém está simplesmente mentindo. Cada um tocou alguma coisa verdadeira. Mas a verdade parcial, quando perde humildade, transforma-se em erro orgulhoso. O escuro não impede apenas a visão; impede também a percepção do limite de cada experiência. A convicção nasce rápido quando a pessoa confunde intensidade com amplitude.',
      'Se houvesse luz, as diferenças poderiam encontrar lugar. A tromba, a perna, a orelha e o dorso seriam reconhecidos como partes de um mesmo animal. Mas, no escuro, cada fragmento compete para ocupar o lugar do todo. A falta de luz torna as pessoas mais certas justamente quando deveriam ser mais cuidadosas.',
      'As descrições se chocaram: calha, leque, pilar, trono. Cada mão trazia evidência para sua certeza, e nenhuma conseguia oferecer às demais a parte que havia tocado.',
      'Rumi encerra a imagem dizendo que uma única vela teria resolvido a disputa. O elefante não mudaria com a luz; mudaria a possibilidade de reunir as partes.'
    ],
    umModoDeOlhar: [
      'O erro não está nas mãos: cada descrição conserva algo verdadeiro. Ele começa quando uma experiência parcial exige o lugar do conjunto. A vela representa uma condição comum de observação, não a vitória de uma pessoa sobre as outras.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Uma percepção pode ser legítima e ainda insuficiente. Reconhecer seu limite não obriga a abandoná-la; permite aproximá-la de outras evidências sem transformar diferença em falsidade imediata.'
    ],
    perguntaParaLevar: 'Que outra perspectiva ajudaria a situar a parte que você consegue perceber?',
  },
  {
    id: 'nasrudin-chave',
    titulo: 'Nasrudin e a Chave Perdida',
    origem: 'Adaptação editorial de anedota de circulação moderna atribuída a Nasrudin',
    tradicao: 'Sufismo popular',
    tempoLeitura: 2,
    sentimentosRelacionados: ['confusão', 'ansiedade', 'autoconhecimento'],
    temas: ['ego', 'ignorância', 'humor', 'busca', 'autoengano'],
    palavrasChave: ['nasrudin', 'chave', 'busca', 'humor', 'autoengano'],
    texto: [
      'Nasrudin estava agachado na rua, procurando alguma coisa debaixo de um poste. A noite já havia caído, e a luz amarela desenhava um círculo pequeno no chão. Ele passava as mãos pela poeira, olhava entre pedras, afastava folhas, voltava ao mesmo lugar. Um vizinho, vendo a cena, aproximou-se com boa vontade e perguntou o que havia acontecido.',
      'Nasrudin respondeu que perdera a chave de casa. O vizinho, solidário, inclinou-se também e começou a ajudar. Por alguns minutos, os dois procuraram em silêncio. A chave não apareceu. O vizinho então fez a pergunta óbvia: “Você tem certeza de que perdeu a chave aqui?” Nasrudin respondeu com tranquilidade: “Não. Perdi lá dentro de casa.”',
      'O vizinho parou, sem entender. Se a chave havia sido perdida dentro de casa, por que procurá-la na rua? Nasrudin apontou para o poste e explicou: ali havia mais luz. A resposta é absurda, e justamente por isso permanece. O humor desmonta a aparência de sensatez. Ele não está procurando onde a chave está; está procurando onde é mais fácil procurar.',
      'O vizinho perguntou como poderiam encontrar na rua uma chave perdida dentro da casa. Nasrudin tornou a apontar para o círculo claro: lá dentro estava escuro demais para procurar.',
      'Por um instante, os dois olharam o chão bem iluminado onde já sabiam que a chave não estava. A busca era mais fácil ali; a porta continuava fechada.'
    ],
    umModoDeOlhar: [
      'A anedota separa intensidade de pertinência. Nasrudin procura de verdade, aceita ajuda e examina cada pedra — mas escolhe o lugar segundo a facilidade de ver, não segundo a possibilidade de encontrar.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Métodos claros e territórios familiares podem manter uma busca ativa longe de seu objeto. O humor aparece porque a inadequação é evidente para o vizinho e perfeitamente razoável para quem prefere a luz.'
    ],
    perguntaParaLevar: 'Que parte de uma busca foi escolhida pela boa iluminação, e não pelo lugar em que algo se perdeu?',
  },
  {
    id: 'cachorro-amarrado-carroca',
    titulo: 'O Cachorro Amarrado à Carroça',
    origem: 'Adaptação editorial de fragmento estoico atribuído a Zenão e Crisipo, preservado por Hipólito',
    tradicao: 'Estoicismo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['ansiedade', 'medo', 'raiva', 'confusão'],
    temas: ['aceitação', 'destino', 'resistência', 'liberdade interior', 'controle'],
    palavrasChave: ['estoicismo', 'carroça', 'cachorro', 'aceitação', 'destino'],
    texto: [
      'Um fragmento da tradição estoica compara a vida a um cachorro amarrado a uma carroça em movimento. A corda permite certa distância, mas não entrega ao animal o rumo do veículo.',
      'Se acompanha a carroça, o cachorro corre por vontade própria ao longo do trajeto que também lhe é imposto. Seu movimento e o movimento do veículo coincidem enquanto a corda permanece frouxa.',
      'Se decide resistir, a carroça não para por causa disso. A mesma ligação que antes permitia acompanhar o passo passa a arrastá-lo.',
      'A imagem termina nesse contraste severo. Em ambos os casos há necessidade; em apenas um deles, segundo o fragmento, a ação voluntária se junta ao curso que não foi escolhido.'
    ],
    umModoDeOlhar: [
      'O fragmento tenta conciliar destino e vontade: o cachorro não escolhe a direção geral, mas pode acompanhar o que ocorrerá de qualquer modo. Essa imagem não decide quais acontecimentos são de fato inevitáveis, questão indispensável antes de aplicá-la a uma situação humana.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Chamar algo de carroça em movimento pode esclarecer um limite real ou encobrir uma possibilidade de intervenção. A margem de liberdade começa também no discernimento entre o que já está dado e o que apenas parece incontornável.'
    ],
    perguntaParaLevar: 'O que lhe permite distinguir um curso inevitável de uma situação que ainda pode ser modificada?',
  },
  {
    id: 'taca-quebrada',
    titulo: 'A Taça Quebrada',
    origem: 'Adaptação editorial de Epicteto, Manual, 3, e Discursos, 3.24',
    tradicao: 'Estoicismo',
    tempoLeitura: 2,
    sentimentosRelacionados: ['saudade', 'luto', 'amor', 'ansiedade'],
    temas: ['apego', 'impermanência', 'perda', 'preparação', 'serenidade'],
    palavrasChave: ['taça', 'quebrada', 'epicteto', 'apego', 'perda'],
    texto: [
      'Epicteto propõe começar pelas coisas pequenas. Ao tomar gosto por uma taça de barro, é preciso lembrar o que ela é: um objeto desse tipo pode quebrar.',
      'A lembrança não impede que alguém a use, cuide dela ou prefira sua forma. Apenas mantém junto do afeto a natureza do objeto, que não recebeu garantia contra queda e desgaste.',
      'Nos Discursos, a imagem reaparece como treino diante da perda. Quando a taça se quebra, é possível dizer que uma taça se quebrou; quando acontece na casa alheia, quase todos reconhecem isso sem espanto.',
      'A dificuldade surge quando a mesma fragilidade atravessa aquilo que alguém chama de seu. O objeto não mudou de natureza ao ser amado; mudou o lugar que ocupava na vida de quem o perdeu.'
    ],
    umModoDeOlhar: [
      'O exercício estoico desloca a descrição possessiva — “minha taça” — para a classe de coisas a que o objeto pertence. Essa mudança pode preparar para a perda, mas não demonstra que todos os vínculos devam ser sentidos como equivalentes a objetos quebráveis.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Conhecer a fragilidade de algo não elimina seu valor nem antecipa por completo a experiência de perdê-lo. A consciência da condição e o vínculo concreto permanecem lado a lado, sem que uma fórmula apague o segundo.'
    ],
    perguntaParaLevar: 'O que muda — e o que não muda — quando a fragilidade é lembrada antes da perda?',
  },
  {
    id: 'grande-inquisidor',
    titulo: 'O Grande Inquisidor',
    origem: 'Adaptação editorial de Fiódor Dostoiévski, Os Irmãos Karamázov, livro V, capítulo 5',
    tradicao: 'Literatura filosófica',
    tempoLeitura: 3,
    sentimentosRelacionados: ['medo', 'confusão', 'raiva', 'falta_de_proposito'],
    temas: ['liberdade', 'religião', 'autoridade', 'segurança', 'responsabilidade'],
    palavrasChave: ['liberdade', 'inquisidor', 'autoridade', 'segurança', 'dostoiévski'],
    texto: [
      'Na Sevilha imaginada por Ivan Karamázov, as fogueiras da Inquisição ainda ardiam quando Cristo apareceu entre o povo. Não anunciou o próprio nome. Caminhou pela praça, e as pessoas abriram passagem como se reconhecessem algo anterior às palavras. Uma criança morta foi trazida para fora da catedral; a mãe se lançou diante dele, e a menina tornou a abrir os olhos.',
      'O cardeal Grande Inquisidor assistiu à cena de longe. Era um homem de noventa anos, alto, com o rosto seco pela idade. Bastou que levantasse um dedo. Os guardas atravessaram a multidão, prenderam Cristo e o levaram para uma cela. O povo que acabara de cercá-lo se curvou diante do cardeal.',
      'À noite, a porta de ferro se abriu. O Inquisidor entrou sozinho, segurando uma lâmpada. Por algum tempo observou o prisioneiro em silêncio. Depois perguntou por que ele voltara. Sua presença, disse, perturbava uma obra construída durante séculos.',
      'O velho falou das tentações no deserto: pão, milagre e poder. Acusou Cristo de haver recusado instrumentos que poderiam tornar a humanidade obediente e tranquila. As pessoas, afirmou, não suportavam a liberdade que lhes fora entregue. Pediam alimento, sinais e uma autoridade diante da qual pudessem depor o peso de escolher.',
      'Enquanto falava, o Inquisidor atravessava a cela em passos curtos. Disse que a instituição corrigira aquela recusa. Daria ordem em lugar de consciência e permitiria que multidões vivessem sem responder pelo próprio caminho. Chamou isso de compaixão. Ainda assim, aproximava a lâmpada do rosto do prisioneiro como quem esperava encontrar uma acusação.',
      'Cristo não respondeu. O velho preferiria desprezo, defesa, qualquer palavra que pudesse combater. O silêncio deixava inteiro o argumento que ele repetira por toda a vida e, ao mesmo tempo, impedia que ele descansasse dentro dele.',
      'Quando terminou, o Inquisidor avisou que o prisioneiro seria queimado na manhã seguinte. Cristo se levantou, aproximou-se e beijou os lábios envelhecidos do homem. O cardeal estremeceu. Abriu a porta da cela e mandou que ele partisse e não voltasse.',
      'Cristo saiu para as ruas escuras. O Inquisidor permaneceu junto à porta, com a lâmpada na mão. Não abandonou sua ideia; mas o beijo continuou ardendo onde nenhuma resposta havia sido dita.'
    ],
    umModoDeOlhar: [
      'O Inquisidor não se apresenta como inimigo da humanidade, mas como alguém que decidiu poupá-la do peso da liberdade. É isso que torna sua defesa perturbadora: a dominação aparece vestida de cuidado, e a obediência, de alívio.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Há proteções que diminuem o medo ao mesmo tempo que estreitam a possibilidade de escolher. O beijo não derrota o argumento do velho nem o converte; apenas introduz uma resposta que sua lógica de controle não consegue absorver.'
    ],
    perguntaParaLevar: 'Que parte da liberdade se torna mais difícil quando alguém oferece decidir em seu lugar?',
  },
  {
    id: 'o-espelho-machado',
    titulo: 'O Espelho',
    origem: 'Adaptação editorial de Machado de Assis, O Espelho, em Papéis Avulsos (1882)',
    tradicao: 'Literatura brasileira',
    tempoLeitura: 3,
    sentimentosRelacionados: ['autoconhecimento', 'solidão', 'confusão', 'insegurança'],
    temas: ['identidade', 'papel social', 'aparência', 'ego', 'máscara'],
    palavrasChave: ['espelho', 'machado', 'identidade', 'máscara', 'aparência'],
    texto: [
      'No conto O Espelho, Machado de Assis apresenta Jacobina, um homem que defende uma teoria curiosa: cada pessoa teria duas almas. Uma alma interior, ligada ao que sente e pensa, e uma alma exterior, formada pelo olhar dos outros, pelos cargos, roupas, títulos e sinais sociais que sustentam uma imagem de identidade. A ideia parece estranha, mas Machado a conduz com ironia até que ela se torne desconfortavelmente reconhecível.',
      'Jacobina conta que, quando jovem, foi nomeado alferes da Guarda Nacional. A farda mudou o modo como as pessoas o tratavam. Parentes, conhecidos e vizinhos passaram a vê-lo com respeito especial. Pouco a pouco, aquele papel social deixou de ser apenas uma função e começou a alimentar sua sensação de existência. Ele já não era apenas Jacobina; era o alferes. A imagem pública começou a servir como espelho da alma.',
      'Em certo momento, fica sozinho em uma casa. Sem a admiração dos outros, sem vozes confirmando sua importância, sem o ambiente social que sustentava a farda, Jacobina começa a se sentir esvaziado. Sua identidade parece perder nitidez. O mundo interior, que deveria bastar, não basta. Ele descobre que parte de si dependia do reflexo devolvido pelos outros.',
      'A cena decisiva acontece diante do espelho. Ao vestir novamente a farda, Jacobina recupera a própria imagem. Não é apenas vaidade externa. É mais inquietante: a roupa reorganiza sua percepção de si. Machado mostra como um papel pode ocupar o centro da pessoa até que ela já não saiba quem é sem ele. O espelho não revela apenas o rosto; revela a dependência da confirmação social.',
      'O conto permanece atual porque todos carregamos alguma farda simbólica: profissão, beleza, competência, reconhecimento, número, prestígio, pertencimento. Machado não condena toda vida social. Ele apenas expõe o perigo de entregar a identidade inteira ao reflexo. Quando o espelho externo desaparece, a pergunta retorna sem delicadeza: ainda há alguém ali?',
      'A genialidade do conto está na ironia sem grito. Jacobina não é um monstro; é um homem comum revelado por uma situação extrema. Justamente por isso incomoda. O espelho machadiano mostra que a vaidade nem sempre aparece como arrogância. Às vezes aparece como dependência silenciosa: a necessidade de ser visto de determinado modo para conseguir sentir-se existente.'
    ],
    umModoDeOlhar: [
      'A farda não funciona apenas como sinal de vaidade: ela recompõe, no espelho, uma figura que Jacobina já não conseguia perceber sozinho. Machado deixa desconfortável a fronteira entre papel social e identidade, sem afirmar que uma alma interior permaneça intacta por baixo de tudo.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Cargos, relações e formas de reconhecimento participam de quem alguém se torna. A dependência aparece quando a ausência desses reflexos torna a própria imagem indistinta, como se nenhum outro modo de presença estivesse disponível.'
    ],
    perguntaParaLevar: 'Quais reflexos sociais hoje participam mais da imagem que você faz de si?',
  },
  {
    id: 'morte-de-ivan-ilitch',
    titulo: 'A Morte de Ivan Ilitch',
    origem: 'Adaptação editorial de Liev Tolstói, A Morte de Ivan Ilitch (1886)',
    tradicao: 'Literatura russa',
    tempoLeitura: 3,
    sentimentosRelacionados: ['luto', 'medo', 'falta_de_proposito', 'tristeza'],
    temas: ['morte', 'sentido', 'vida falsa', 'arrependimento', 'verdade'],
    palavrasChave: ['ivan ilitch', 'morte', 'sentido', 'tolstói', 'vida'],
    texto: [
      'Ivan Ilitch viveu uma vida que, por fora, parecia bem organizada. Estudou, trabalhou, casou-se, buscou uma casa confortável, subiu na carreira e aprendeu a mover-se nos ambientes sociais com a correção esperada. Não era um homem monstruoso. Era, de certo modo, comum: alguém que confundiu respeitabilidade com verdade, conveniência com felicidade e aparência de sucesso com vida bem vivida.',
      'Tolstói acompanha esse homem quando a doença começa a desfazer a superfície. Primeiro, Ivan tenta tratar a dor como um problema técnico, algo a ser resolvido por médicos, diagnósticos e linguagem socialmente aceitável. Mas a dor cresce. O corpo, antes tratado quase como instrumento de posição e rotina, torna-se uma presença impossível de ignorar. A morte deixa de ser uma ideia distante e passa a ocupar o quarto.',
      'O mais terrível não é apenas morrer. É perceber, diante da morte, que talvez a vida tenha sido vivida de maneira falsa. Ivan começa a suspeitar que muitas decisões foram tomadas para parecerem adequadas aos olhos dos outros. Aquilo que sustentava sua identidade — cargo, casa, etiqueta, opinião social — perde força diante da pergunta simples e devastadora: “E se tudo isso não tiver sido o essencial?”',
      'Ao redor dele, quase todos tentam manter a mentira social. Falam como se a morte não estivesse ali. Tratam o sofrimento como inconveniente. Apenas alguns gestos simples de compaixão conseguem romper essa falsidade. A presença do criado Guerássim, que não foge da realidade da morte, oferece a Ivan algo raro: uma relação menos mascarada, menos decorada, mais humana.',
      'Nas últimas horas, Ivan vê o filho junto à cama e percebe o sofrimento dele e da esposa. Em vez de continuar exigindo que compreendam sua dor, deseja poupá-los. Tenta pedir perdão, mas já não consegue pronunciar todas as palavras.',
      'O medo da morte desaparece quando ele deixa de procurá-la. No lugar onde esperava encontrar o fim, percebe luz. Para os que observam, sua agonia ainda dura algum tempo; para Ivan, a morte terminou antes que seu corpo ficasse imóvel.'
    ],
    umModoDeOlhar: [
      'A novela não reduz a vida de Ivan a uma máscara facilmente removida. Sua respeitabilidade organizou escolhas, relações e hábitos durante anos. A doença desfaz essa ordem aos poucos, enquanto o gesto de Guerássim e a presença do filho introduzem outra medida de verdade.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'A proximidade da morte altera o valor de coisas antes consideradas indispensáveis, mas o texto não oferece esse confronto como técnica de aperfeiçoamento. Ele acompanha uma consciência cercada por dor, negação social e breves relações sem fingimento.'
    ],
    perguntaParaLevar: 'Que diferença Guerássim e o filho introduzem no quarto onde quase todos evitam reconhecer o que acontece?',
  },
  {
    id: 'jornada-do-heroi',
    titulo: 'A Jornada do Herói',
    origem: 'Síntese editorial do monomito em Joseph Campbell, O Herói de Mil Faces (1949)',
    tradicao: 'Mitologia comparada',
    tempoLeitura: 2,
    sentimentosRelacionados: ['coragem', 'falta_de_proposito', 'medo', 'autoconhecimento'],
    temas: ['transformação', 'coragem', 'chamado', 'travessia', 'retorno'],
    palavrasChave: ['herói', 'jornada', 'transformação', 'chamado', 'coragem'],
    texto: [
      'A Jornada do Herói não é um conto antigo único. É um modelo comparativo formulado por Joseph Campbell a partir de narrativas diferentes. Nesta síntese, uma figura deixa o mundo conhecido depois de receber um chamado que também pode recusar.',
      'Ao atravessar o limiar, encontra provas, aliados e adversários. A ordem varia de mito para mito; alguns episódios desaparecem, outros se repetem, e nem toda personagem retorna pelo mesmo caminho.',
      'No ponto mais profundo da travessia, aquilo que estava em jogo muda de forma. A vitória pode ser um objeto, um conhecimento, uma reconciliação ou a simples sobrevivência ao encontro.',
      'O retorno cria um segundo conflito. O que foi encontrado fora precisa atravessar de volta o limiar e encontrar lugar no mundo inicial, que também pode resistir à mudança.',
      'O mapa termina onde uma história particular começaria a recuperar seus nomes, lugares e consequências. Nenhuma etapa substitui o trabalho de ouvir o mito do qual foi abstraída.'
    ],
    umModoDeOlhar: [
      'O monomito torna comparáveis movimentos de muitas narrativas, mas essa capacidade também é seu limite. Quando o mapa parece universal demais, diferenças culturais, personagens que não retornam e histórias sem triunfo podem ser forçadas a caber no mesmo percurso.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Um modelo pode organizar uma experiência sem provar que ela seja chamado, prova ou transformação necessária. Às vezes o mapa oferece linguagem; em outras, encobre aquilo que não segue uma jornada reconhecível.'
    ],
    perguntaParaLevar: 'Que parte de uma experiência se esclarece com este mapa, e que parte permanece fora dele?',
  },
  {
    id: 'a-sombra-jung',
    titulo: 'A Sombra',
    origem: 'Alegoria editorial baseada no conceito de sombra em C. G. Jung',
    tradicao: 'Psicologia analítica',
    tempoLeitura: 2,
    sentimentosRelacionados: ['autoconhecimento', 'raiva', 'culpa', 'confusão', 'insegurança'],
    temas: ['autoconhecimento', 'sombra', 'projeção', 'integração', 'verdade interior'],
    palavrasChave: ['sombra', 'jung', 'projeção', 'culpa', 'raiva'],
    texto: [
      'Um retratista decidiu pintar a própria imagem apenas com os traços que admirava. Corrigiu a boca, apagou o cansaço dos olhos e cobriu de tinta clara tudo o que lhe parecia indigno de permanecer.',
      'Ao fim do dia, levou o quadro para perto da janela. A luz tornou a figura nítida, mas projetou atrás da tela uma sombra irregular, formada também pelas camadas que ele tentara esconder.',
      'O retratista mudou a posição da lâmpada. A sombra atravessou outra parede. Aproximou a tela da luz; o contorno escuro cresceu. Nenhum desses movimentos devolveu ao quadro os traços apagados.',
      'Então virou a pintura e examinou o verso. Viu relevos, correções e cores acumuladas que a superfície lisa não mostrava. Não rasgou o retrato nem declarou belo tudo o que encontrou.',
      'Na manhã seguinte, tornou a pintar. Algumas linhas permaneceram, outras foram revistas. A sombra continuou no chão, agora sem precisar fingir que viera de outra pessoa.'
    ],
    umModoDeOlhar: [
      'Esta alegoria não é uma história escrita por Jung. Ela traduz editorialmente a tensão entre a imagem consciente e aspectos recusados da personalidade. Virar o quadro significa reconhecer material ocultado, não tratá-lo como verdade superior nem obedecer a tudo o que aparece.'
    ],
    oQueTalvezEstejaPedindoParaSerVisto: [
      'Uma reação ao outro pode conter projeção e ainda responder a algo real no outro. A imagem da sombra convida ao exame da própria participação sem transformar todo conflito externo em problema interior.'
    ],
    perguntaParaLevar: 'Que traço do retrato merece ser examinado também pelo verso, sem decidir antes o que ele significa?',
  },
];
})(typeof window !== 'undefined' ? window : globalThis);

...
| `entreSabiosContosVistos` | `script.js` e `js/features/tales.js`; `viewedTaleKeys` | `selectTale()` registra `contexto::id`; `markTaleAsRead()` registra `read::id` depois da apresentação | `getRankedTalesForState()` consulta somente `contexto::id`; a interface consulta somente `read::id` para mostrar “Lido” | `localStorage`; até 120 entradas, preservando no máximo um marcador explícito por conto | Rotação contextual e indicação local de leitura, com semânticas separadas | Autoridade contextual dos contos e registro explícito de leitura | Manter na mesma chave; não interpretar contexto, recência ou ciclo como leitura e não integrar ao histórico das reflexões |
